...
// ═══════════════════════════════════════════════════════════════
// openvibe.deals — OAuth2 CLIENT session layer (sign-in via OpenVibe.Network SSO)
//
// The same session layer OpenVibe.Community uses (server/auth/routes.js there, MIT), with
// Deals' client id: registered in the Network as OAuth client `deals`, redirect
// https://openvibe.deals/auth/callback.
//
...
//
// Cookies (host-only for openvibe.deals):
        ...
        for (const base of [config.networkInternalUrl, config.networkUrl]) {
            ...
            try {
                ...
                if (jwks.public_key) {
                    ...
                    console.log(`[Deals] Network public key loaded from ${base} (${jwks.algorithm || 'RS256'})`);
                    return client.publicKey;
                }
            } catch (err) {
                console.warn(`[Deals auth] JWKS fetch failed from ${base}: ${err.message}`);
            }
        }
        return null;
    }
...
function createAuthRoutes(config, auth) {
    ...
    router.get('/callback', async (req, res) => {
        ...
        try {
            ...
            console.error('[Deals auth] Code exchange failed:', err.message);
    ...
    router.post('/fedcm', fedcmBody, fedcmBodyError, async (req, res) => {
        ...
        try {
            ...
            console.error('[Deals auth] FedCM exchange failed:', err.message);
    ...
    router.post('/refresh', async (req, res) => {
        ...
        try {
            ...
            console.error('[Deals auth] Refresh failed:', err.message);
